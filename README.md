...
| `path_separator` | Set this option to force a different path separator (like `-` instead of `/`).  Useful if you have edited `ActionDispatch::Routing::SEPARATORS`.  This must be a single character which is not used (without escaping) in parameters. | `/` |
